...
def load_woe_data():
    path = os.path.join(PROCESSED_PATH, 'features_woe.csv')
    if not os.path.exists(path):
        raise FileNotFoundError(
            f'features_woe.csv not found in {PROCESSED_PATH}. '
...
def show_woe_columns(df):
    ...
    print(f"\nWOE columns found in file: {woe_cols}")
    return woe_cols
...
def drop_correlated(df, feature_cols, threshold=0.85):
    print(f"\nChecking correlations (threshold={threshold})...")
...
def deduplicate_customers(df):
    print("\nFixing duplicate customer rows and SK_ID_CURR type...")
...
def run():
    ...
    df = load_woe_data()
    print(f"Loaded features_woe.csv — shape: {df.shape}")

    show_woe_columns(df)
    available, missing, mapped = match_expected_columns(df)
    ...
    if not available:
        raise ValueError(
            'No expected WOE features were found in features_woe.csv. '
            'Cannot build features_final.csv.'
        )

    final_cols = drop_correlated(df, available)
    df = deduplicate_customers(df)
    final_df = build_final_matrix(df, final_cols)

    final_df.to_csv(os.path.join(PROCESSED_PATH, 'features_final.csv'), index=False)
    ...
    unique_customers = final_df['SK_ID_CURR'].nunique()
